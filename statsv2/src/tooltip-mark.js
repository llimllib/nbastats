...
// which I got via:
// https://github.com/observablehq/plot/issues/4
export function tooltip(Plot) {
  ...
  class Tooltip extends Plot.Mark {
    ...
    render(
      ...
      setTimeout(() => {
        ...
        tooltip =
          this.tooltip ||
          (this.tooltip = select(owner.parentElement)
...
function arrowTextFormatter(div, value, { direction } = {}) {
  const g = select(div).append("svg").style("overflow", "visible");
